...
type NavigationMode = 'guest' | 'user' | 'admin'

export const navigationConfig: Record<NavigationMode, NavigationItem[]> = {
  guest: [
...
export const userMenuConfig: { items: UserMenuItem[]; adminItem: UserMenuItem } = {
  items: [
    ...
    { name: '修改密码', path: '/user/change-password', icon: 'shield' },
  ],
  adminItem: { name: '管理后台', path: '/admin', icon: 'shield' },
};

export function getNavigationItems(userRole: string): NavigationItem[] {
  return navigationConfig[userRole as NavigationMode] || navigationConfig.guest;
}
