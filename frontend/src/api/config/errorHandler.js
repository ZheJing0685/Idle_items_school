...
class ErrorHandler {
  ...
  static handleHttpError(error) {
    if (error.response) {
      ...
      return { code: ERROR_CODES.TIMEOUT_ERROR, message: this.getErrorMessage(ERROR_CODES.TIMEOUT_ERROR) };
    } else if (error.message?.includes('Network Error')) {
      return { code: ERROR_CODES.NETWORK_ERROR, message: this.getErrorMessage(ERROR_CODES.NETWORK_ERROR) };
    } else {
      return { code: ERROR_CODES.UNKNOWN_ERROR, message: this.getErrorMessage(ERROR_CODES.UNKNOWN_ERROR) };
    }
  }
  ...
  static clearAuthStorage() {
    ...
    document.cookie = 'user:refreshToken=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;';
  }
}
