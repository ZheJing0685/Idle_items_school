...
describe('Order API', () => {
  ...
  describe('updateShippingInfo', () => {
    it('should call post with correct url and shipping params', async () => {
      const mockResponse = { code: 200, data: null };
      mockPost.mockResolvedValue(mockResponse);

      const result = await orderApi.updateShippingInfo(
        1,
        'SF1234567890',
        '顺丰速运',
      );

      expect(mockPost).toHaveBeenCalledWith('/orders/1/shipping', null, {
        params: { trackingNumber: 'SF1234567890', shippingCompany: '顺丰速运' },
      });
      expect(result).toEqual(mockResponse);
    });
  });
