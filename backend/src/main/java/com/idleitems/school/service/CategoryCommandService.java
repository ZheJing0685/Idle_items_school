        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    if (parts.length > 2 && !parts[2].trim().isEmpty()) category.setParentId(Long.parseLong(parts[2].trim()));
                    if (parts.length > 3 && !parts[3].trim().isEmpty()) category.setLevel(Integer.parseInt(parts[3].trim()));
                    if (parts.length > 4 && !parts[4].trim().isEmpty()) category.setSort(Integer.parseInt(parts[4].trim()));
                    if (parts.length > 5) category.setStatus("ACTIVE".equalsIgnoreCase(parts[5].trim()));
