package com.idleitems.school.module.dispute.service;
...
public class DisputeCommandService {
        ...
        if ("RESOLVED".equals(action)) {
            ...
            if ("APPROVE_REFUND".equals(result) || "PARTIAL_REFUND".equals(result)) {
                ...
                if (order != null) {
                    ...
                    if ("APPROVE_REFUND".equals(result)) {
                        refundAmount = actualRefundAmount != null ? actualRefundAmount : order.getPrice();
                    } else {
                        // PARTIAL_REFUND 必须指定退款金额
                        if (actualRefundAmount == null || actualRefundAmount.compareTo(BigDecimal.ZERO) <= 0) {
                            throw new BusinessException(ErrorCode.BAD_REQUEST, "部分退款必须指定有效的退款金额");
                        }
                        if (actualRefundAmount.compareTo(order.getPrice()) > 0) {
                    ...
                    dispute.setActualRefundAmount(refundAmount);
                }
            }
    ...
    private String generateDisputeNo() {
        // 使用 UUID 前8位 + 时间戳后6位，兼顾唯一性和不可预测性
        String uuid = java.util.UUID.randomUUID().toString().replace("-", "").substring(0, 8).toUpperCase();
        String timeSuffix = String.format("%06d", System.currentTimeMillis() % 1_000_000);
        return "DS" + timeSuffix + uuid;
    }
