package com.idleitems.school.module.file.service.impl;
...
public class FileServiceImpl implements FileService {
    ...
    public String uploadFile(MultipartFile file, String directory) throws IOException {
        ...
        if (file.getSize() > maxSize) {
            throw new BusinessException(ErrorCode.FILE_SIZE_EXCEEDED, "文件大小不能超过" + (maxSize / 1024 / 1024) + "MB");
        }

        String fileName = file.getOriginalFilename();
        if (fileName == null || !isAllowedFileType(fileName)) {
            Set<String> allowedExtensions = getAllowedExtensions();
            throw new BusinessException(ErrorCode.FILE_TYPE_NOT_ALLOWED, "只支持" + allowedExtensions.stream()
        ...
        try (InputStream inputStream = file.getInputStream()) {
            if (ImageIO.read(inputStream) == null) {
                throw new BusinessException(ErrorCode.FILE_TYPE_NOT_ALLOWED, "无效的图片文件");
            }
        }
